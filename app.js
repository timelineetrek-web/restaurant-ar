...
function placeBurger() {
  ...
  showControls(true);
  status.textContent = 'Burger placed — use the buttons below';
}

function selectBurger(value = true) {
  selected = value;
  selectionRing.visible = value && placed;
  if (value && placed) status.textContent = 'Burger selected — use the buttons below';
}
...
rotateLeft.addEventListener('click', buttonAction(() => {
  burger.rotation.y -= Math.PI / 12;
}));

rotateRight.addEventListener('click', buttonAction(() => {
  burger.rotation.y += Math.PI / 12;
}));

scaleDown.addEventListener('click', buttonAction(() => {
  const next = THREE.MathUtils.clamp(burger.scale.x * 0.82, 0.16, 0.75);
  burger.scale.setScalar(next);
}));

scaleUp.addEventListener('click', buttonAction(() => {
  const next = THREE.MathUtils.clamp(burger.scale.x * 1.22, 0.16, 0.75);
  burger.scale.setScalar(next);
}));
...
removeButton.addEventListener('click', (event) => {
  ...
  selectionRing.visible = false;
  showControls(false);
  status.textContent = reticle.visible ? 'Surface found — tap the blue ring to place' : 'Move your phone until the blue ring appears';
});

// DOM overlay controls receive normal browser events. A tap anywhere on the
// AR overlay while the placement ring is visible places the burger. The
// control bar stops propagation so its buttons never place a new burger.
document.body.addEventListener('pointerup', (event) => {
  if (!renderer.xr.isPresenting || placed || !reticle.visible) return;
  if (event.target.closest('#ar-controls, #ar-button')) return;
  placeBurger();
}, { passive: true });
...
renderer.setAnimationLoop((time, frame) => {
  if (frame && hitTestSource && localReferenceSpace) {
    ...
    if (results.length) {
      ...
      if (!placed) status.textContent = 'Surface found — tap the blue ring to place';
    } else {
      reticle.visible = false;
